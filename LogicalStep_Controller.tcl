...
if {[is_project_open]} {
	if {[string compare $quartus(project) "LogicalStep_Controller"]} {
		puts "Project LogicalStep_Controller is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists LogicalStep_Controller]} {
		project_open -revision LogicalStep_Controller_top LogicalStep_Controller
	} else {
		project_new -revision LogicalStep_Controller_top LogicalStep_Controller
	}
...
# 	set_global_assignment -name QIP_FILE LogicalStep_Controller/synthesis/LogicalStep.qip
#	set_global_assignment -name VERILOG_FILE LogicalStep_Controller_top.v
	set_global_assignment -name TCL_SCRIPT_FILE LogicalStep_Controller.tcl
